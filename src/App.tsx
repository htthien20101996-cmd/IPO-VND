      ...
      {!isFullyPaid && (
        <div className="mb-8">
          <div className="flex justify-between items-center mb-4">
             ...
             {canSwitchMethod && (
               <button 
                 onClick={() => onUpdateMethod(registration.id, isStockAccount ? 'BANK_TRANSFER' : 'STOCK_ACCOUNT')}
                 className="text-[10px] font-black text-blue-600 uppercase flex items-center gap-1.5 bg-blue-50 px-3 py-1.5 rounded-xl border border-blue-100 hover:bg-blue-100 transition-all shadow-sm"
               >
                 <RefreshCw size={12} />
          ...
          {isStockAccount ? (
            <div className="bg-blue-50 border border-blue-100 rounded-3xl p-6">
              ...
              <p className="text-[10px] text-gray-600 leading-relaxed italic mb-0">
            ...
            <div className="space-y-4">
              <div className="bg-white border-2 border-orange-500 rounded-3xl p-6 shadow-sm">
                ...
                <div className="space-y-3 bg-gray-50 p-4 rounded-2xl text-[10px]">
              ...
              <div className="bg-emerald-50 border border-emerald-100 rounded-3xl p-6">
                <h4 className="text-[10px] font-black text-emerald-700 uppercase tracking-widest mb-4 flex items-center gap-2">
                  <Upload size={14} />
                  Xác nhận nộp tiền (Ủy nhiệm chi)
                </h4>
                ...
                {receiptImage ? (
                  <div className="relative group">
                    <img src={receiptImage} alt="Receipt" className="w-full h-48 object-cover rounded-2xl border-2 border-dashed border-emerald-200" />
                    <button 
                  ...
                  <button 
                    onClick={() => fileInputRef.current?.click()}
                    className="w-full h-32 border-2 border-dashed border-emerald-200 rounded-2xl flex flex-col items-center justify-center gap-2 hover:bg-emerald-100/50 transition-all group"
                  >
                    ...
                    <span className="text-[10px] font-black text-emerald-600 uppercase tracking-widest">Tải lên ảnh ủy nhiệm chi</span>
                    <span className="text-[8px] text-emerald-400 font-medium uppercase">Hỗ trợ JPG, PNG (tối đa 5MB)</span>
                  </button>
                )}
                
                {receiptImage && (
                  <button className="w-full mt-4 py-3 bg-emerald-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg shadow-emerald-100 hover:bg-emerald-700 transition-all">
                    Gửi xác nhận nộp tiền
                  </button>
                )}
              </div>
            </div>
